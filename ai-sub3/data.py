...
def load_data():
    ...
    train_q , train_a, test_q, test_a = train_test_split(data["Q"], data["A"], test_size=0.2)
    return train_q, train_a, test_q, test_a

# Req 1-1-2. 텍스트 데이터에 정규화를 사용하여 ([~.,!?\"':;)(]) 제거
def prepro_noise_canceling(data):
    return re.sub(changeFilter, "", data)
...
def enc_processing(value, dictionary):
    ...
    # 문장의 길이를 저장할 배열 초기화
    seq_len = []
    value = value.apply(tokenizing_data)
    for seq in value:
        seq_index = []
        for word in seq:
...
def dec_target_processing(value, dictionary):
    ...
    for seq in value:
        # 하나의 seq에 index를 저장할 배열 초기화
        seq_index = [dictionary[word] for word in seq]
        # 문장 제한 길이보다 길어질 경우 뒤에 토큰을 제거
...
def pred_next_string(value, dictionary):
    sentenceString = []
    for v in value:
        sentenceString = [dictionary[index] for index in v]
    answer = ""
