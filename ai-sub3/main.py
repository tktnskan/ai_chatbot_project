...
def bleu_compute(references, hypothesis):
    ...
    km = Kkma()
    return sentence_bleu(km.morphs(references), km.morphs(hypothesis), smoothing_function=sf.method1)

# Req. 1-5-2. rouge score 계산 함수
def rouge_compute(references, hypothesis):
    rouge = Rouge()
    return rouge.get_scores(references, hypothesis)["rouge-l"]["f"]
